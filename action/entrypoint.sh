...
if [ "$INPUT_COMMIT" = 'true' ]; then
    ...
    if [ "$INPUT_PULL_REQUEST" = 'true' ]; then
        ...
        gh pr create --head "changelog-${INPUT_VERSION}" --title "Update changelog for ${INPUT_VERSION}" --body "This PR was automatically created by [changelog-from-release](https://github.com/BenGercuj/changelog-from-release) action for ${INPUT_VERSION}"
